...
const AddToCartButton = ({
  ...
  const handleClickAddToCart = useCallback(() => {
    if (selectedIsUserLoggedIn === false) {
      ...
      dispatch(
        addToCart({ id, name, description, image, price, stock })
      );
      navigate("/cart");
    }
  }, [
  ...
  const isInTheCart = useMemo(() => {
    return selectedCartTransaction.some((product) => product.id === id);
  }, [selectedCartTransaction, id]);
  ...
  return (
    ...
    >
      ...
      {isInTheCart ? "GO TO CART" : "ADD TO CART"}
    </button>
  );
};
